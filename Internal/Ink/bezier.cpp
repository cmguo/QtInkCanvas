...
bool Bezier::ExtendingRange(double error, CuspData & data, int from, int next_cusp, int & to, bool & cusp, bool & done)
{
    to++;
...
bool Bezier::AddBezierSegment(CuspData & data, int from, QPointF & tanStart, int to, QPointF & tanEnd)
{
    ...
    return AddLeastSquares(data, from, tanStart, to, tanEnd);
}
...
bool Bezier::ConstructFromData(CuspData & data, double fitError)
{
    ...
    if (DBL_EPSILON > fitError)
        fitError = 0.03 * (data.Distance() * StrokeCollectionSerializer::HimetricToAvalonMultiplier);
    ...
    for (int from = 0; !done; from = to)
    {
        if (is_a_cusp)
        {
            ...
            if (!data.Tangent(tanStart, from, prev_cusp, next_cusp, false, true))
            {
                return false;
            }
        }
        ...
        while (ExtendingRange(fitError, data, from, next_cusp, to, is_a_cusp, done));

        // Find the tangent
        if (!data.Tangent(tanEnd, to, prev_cusp, next_cusp, true, is_a_cusp))
        {
            return false;
        }

        // Add bezier segment
        if (!AddBezierSegment(data, from, tanStart, to, tanEnd))
        {
            return false;
        }
    }
...
void Bezier::AddParabola(CuspData & data, int from)
{
...
void Bezier::AddLine(CuspData & data, int from, int to)
{
    const double third = 1.0 / 3.0;
...
bool Bezier::CoCubic(CuspData & data, int i[], double fitError)
{
