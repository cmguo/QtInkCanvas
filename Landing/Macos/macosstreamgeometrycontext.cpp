#include "macosstreamgeometrycontext.h"
...
MacosStreamGeometryContext::MacosStreamGeometryContext(StreamGeometry * geometry)
{
    geometry_ = geometry;
    path_ = NSBezierPathWrapper_new();
}

MacosStreamGeometryContext::~MacosStreamGeometryContext()
{
    if (path_) {
        NSBezierPathWrapper_delete(path_);
        path_ = nullptr;
    }
}

void MacosStreamGeometryContext::BeginFigure(const Point &startPoint, bool, bool)
{
    //Debug::Log("BeginFigure (%lf,%lf)", startPoint.X(), startPoint.Y());
    NSBezierPathWrapper_moveToPoint(path_, startPoint.X(), startPoint.Y());
}

void MacosStreamGeometryContext::LineTo(const Point &point, bool, bool)
{
    //Debug::Log("LineTo (%lf,%lf)", point.X(), point.Y());
    NSBezierPathWrapper_addLineToPoint(path_, point.X(), point.Y());
}

void MacosStreamGeometryContext::QuadraticBezierTo(const Point &point1, const Point &point2, bool, bool)
{
    //Debug::Log("QuadraticBezierTo (%lf,%lf) (%lf,%lf)", point1.X(), point1.Y(), point2.X(), point2.Y());
    // point2: control point
    NSBezierPathWrapper_addQuadCurveToPoint(path_, point2.X(), point2.Y(), point1.X(), point1.Y());
}

void MacosStreamGeometryContext::BezierTo(const Point &point1, const Point &point2, const Point &point3, bool, bool)
{
    //Debug::Log("BezierTo (%lf,%lf) (%lf,%lf) (%lf,%lf)", point1.X(), point1.Y(), point2.X(), point2.Y(), point3.X(), point3.Y());
    NSBezierPathWrapper_addCurveToPoint(path_, point3.X(), point3.Y(), point1.X(), point1.Y(), point2.X(), point2.Y());
}

void MacosStreamGeometryContext::PolyLineTo(const List<Point> &points, bool isStroked, bool isSmoothJoin)
{
...
void MacosStreamGeometryContext::PolyQuadraticBezierTo(const List<Point> &points, bool isStroked, bool isSmoothJoin)
{
...
void MacosStreamGeometryContext::PolyBezierTo(const List<Point> &points, bool isStroked, bool isSmoothJoin)
{
...
void MacosStreamGeometryContext::ArcTo(const Point &point, const Size &size, double rotationAngle, bool isLargeArc, SweepDirection sweepDirection, bool, bool)
{
    double lx = 0, ly = 0;
    NSBezierPathWrapper_currentPoint(path_, &lx, &ly);
    Point lastp(lx, ly);
    Point curtp = point;
    ...
    NSBezierPathWrapper_addArcWithCenter(path_, c.X(), c.Y(), size.Width(),
                               t1, t2, sweepDirection == SweepDirection::Clockwise);
}

void MacosStreamGeometryContext::Close()
{
    StreamGeometryContext::Close();
}

void MacosStreamGeometryContext::SetClosedState(bool)
{

}

void MacosStreamGeometryContext::DisposeCore()
{
